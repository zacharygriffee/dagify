...
Note: set() returns a promise that resolves on the next tick, allowing you to optionally await state propagation.
...
setIdGenerator(fn)
(Legacy) Customize node id generation. Note: This is less relevant now since node ids are derived from Buffer keys using z32 encoding.
