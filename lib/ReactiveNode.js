import { Observable, Subject, skip, take } from './rxjs/rxjsPrebuilt.js';
import { fromObservable } from "./util/fromObservable.js";
import { isRxObservable } from "./rxjs/isRxObservable.js";
import { deepEqual } from "./util/deepEqual.js";

let idGenerator = () => `Node-${Math.random().toString(36).slice(2, 9)}`;

/**
 * Sets the function used to generate unique node IDs.
 *
 * @param {function} generator - A function that returns a string identifier for nodes.
 */
const setIdGenerator = (generator) => idGenerator = generator;

/**
 * Represents a reactive node in a directed acyclic graph (DAG).
 * Supports computed values, subscriptions, error handling, and completion.
 *
 * @class ReactiveNode
 * @extends {Subject}
 */
class ReactiveNode extends Subject {
    /**
     * A set of ReactiveNodes pending updates.
     *
     * @static
     * @type {Set<ReactiveNode>}
     */
    static pendingUpdates = new Set();

    /**
     * Indicates whether an update is currently being processed.
     *
     * @static
     * @type {boolean}
     */
    static updating = false;

    /**
     * Flag to indicate if batch mode is enabled.
     *
     * @static
     * @type {boolean}
     */
    static batchMode = false;

    /**
     * Indicates that this node is a Dagify node.
     *
     * @type {boolean}
     */
    get isDagifyNode() {
        return true;
    }

    /**
     * Indicates whether this node is asynchronous. For both computed and non-computed nodes,
     * this flag is set to true if the node's value is derived from an asynchronous source
     * (observable or promise).
     *
     * @type {boolean}
     */
    get isAsync() {
        return !!this._isAsync;
    }

    /**
     * Indicates whether this node is computed (i.e. if it was created with a computation function).
     *
     * @type {boolean}
     */
    get isComputed() {
        return !!this.fn;
    }

    /**
     * Gets the unique identifier for this node.
     *
     * This getter returns the node's ID by calling {@link getId}.
     * Users can override {@link getId} to customize the node's ID generation.
     *
     * @readonly
     * @returns {string|number} The unique identifier for this node.
     */
    get id() {
        return this.getId();
    }

    /**
     * Returns the unique identifier for this node.
     *
     * If the identifier has not yet been generated, it is created using the global/custom
     * `idGenerator` function and stored in the private `_id` property. This method can be
     * overridden by users to implement custom ID generation logic.
     *
     * @returns {string|number} The unique identifier for this node.
     */
    getId() {
        return this._id ||= idGenerator();
    }

    /**
     * Creates a new ReactiveNode.
     *
     * @param {Function|*} fnOrValue - A function for a computed node or a direct value.
     * @param {Array<ReactiveNode|Composite|Observable>} [dependencies=[]] - An array of dependency nodes, composite nodes, or observables.
     *        If any dependency is an observable, it is wrapped as a ReactiveNode.
     */
    constructor(fnOrValue, dependencies = []) {
        if (!Array.isArray(dependencies)) dependencies = [dependencies];
        super();

        /**
         * Set of subscriber observers.
         * @type {Set<Object>}
         */
        this.subscribers = new Set();

        /**
         * Flag indicating whether the node has been completed.
         * @type {boolean}
         * @private
         */
        this._completed = false;

        // Wrap any observable dependency into a ReactiveNode.
        this.dependencies = dependencies.map(dep =>
            isRxObservable(dep) ? fromObservable(dep, undefined) : dep
        );

        /**
         * Holds the subscription for an observable value.
         * @type {Object|null}
         * @private
         */
        this._observableSubscription = null;

        // If fnOrValue is a function, this node is computed.
        if (typeof fnOrValue === 'function') {
            /**
             * Computation function used to derive the node's value.
             * @type {Function}
             */
            this.fn = fnOrValue;

            /**
             * The current computed value of the node.
             * @type {*}
             */
            this.value = undefined;

            /**
             * Array of unsubscribe functions for dependency subscriptions.
             * @type {Array<Function>}
             * @private
             */
            this._dependencySubscriptions = this._initializeDependencySubscriptions(this.dependencies);

            this.compute();
        } else {
            this.fn = null;
            // For non-computed (stateful) nodes, if the provided value is asynchronous (an observable),
            // subscribe to it and mark the node as asynchronous.
            if (isRxObservable(fnOrValue)) {
                this._isAsync = true;
                this._subscribeToObservable(fnOrValue);
            } else {
                this._isAsync = false;
                this.value = fnOrValue;
            }
        }
    }

    /**
     * Schedules an update for the given ReactiveNode.
     *
     * In batch mode, the node is simply added to the pending updates set.
     * Otherwise, if an update is not already in progress, a microtask is scheduled to compute
     * all pending nodes.
     *
     * @param {ReactiveNode} node - The node to update.
     * @static
     */
    static scheduleUpdate(node) {
        if (ReactiveNode.batchMode) {
            ReactiveNode.pendingUpdates.add(node);
            return;
        }
        ReactiveNode.pendingUpdates.add(node);
        if (!ReactiveNode.updating) {
            ReactiveNode.updating = true;
            queueMicrotask(() => {
                ReactiveNode.pendingUpdates.forEach((n) => n.compute());
                ReactiveNode.pendingUpdates.clear();
                ReactiveNode.updating = false;
            });
        }
    }

    /**
     * Executes a function in batch mode, deferring updates until the batch completes.
     *
     * @param {Function} fn - A function containing multiple node update operations.
     * @static
     */
    static batch(fn) {
        ReactiveNode.batchMode = true;
        fn();
        ReactiveNode.batchMode = false;
        ReactiveNode.pendingUpdates.forEach((n) => n.compute());
        ReactiveNode.pendingUpdates.clear();
    }

    /**
     * Computes the node's value using its computation function and dependency values.
     * Notifies subscribers with the new value or an error if one occurs.
     *
     * @returns {void}
     */
    compute() {
        if (!this.fn) return;
        try {
            // Gather dependency values.
            const depValues = this.dependencies.map(dep => dep.value);

            // Call the computed function.
            const result = this.fn.length >= 2
                ? this.fn(depValues, this.dependencies)
                : this.fn(depValues);

            // Reset the async flag.
            this._isAsync = false;

            // Handle asynchronous cases.
            if (result && typeof result.subscribe === 'function') {
                this._isAsync = true;
                if (this._asyncSubscription) {
                    this._asyncSubscription.unsubscribe();
                }
                this._asyncSubscription = result.subscribe(
                    newValue => this._setValue(newValue),
                    err => this.error(err)
                );
            } else if (result && typeof result.then === 'function') {
                this._isAsync = true;
                Promise.resolve(result)
                    .then(newValue => this._setValue(newValue))
                    .catch(err => this.error(err));
            } else {
                // Use a cache to avoid redundant updates.
                if (!deepEqual(this._lastComputed, result)) {
                    this._lastComputed = result;
                    this._setValue(result);
                }
            }
            // Clear any previous error.
            this._error = undefined;
        } catch (err) {
            this._error = err;
            this._notifyAll('error', err);
        }
    }

    /**
     * Sets a new value for a non-computed node.
     *
     * If the new value is an observable, the node will subscribe to it (switching out any previous observable).
     * Otherwise, any running observable is canceled and the static value is set.
     *
     * @param {*} newValue - The new value to set.
     * @throws {Error} Throws an error if attempting to set a computed node.
     */
    set(newValue) {
        if (this.fn) throw new Error("Cannot manually set a computed node");

        if (isRxObservable(newValue)) {
            // Switch to the new observable (similar to switchMap behavior)
            this._subscribeToObservable(newValue);
        } else {
            // Cancel any running observable subscription and set the static value.
            if (this._observableSubscription) {
                this._observableSubscription.unsubscribe();
                this._observableSubscription = null;
            }
            this._setValue(newValue);
        }
    }

    /**
     * Triggers an update for the node.
     * For computed nodes, this will recalculate the value; for non-computed nodes, it will set the new value.
     *
     * @param {*} value - The value to be processed.
     */
    next(value) {
        this.fn ? this.compute() : this.set(value);
    }

    /**
     * Subscribes an observer to the node.
     *
     * @param {Object|Function} observer - An observer object with `next`, `error`, and `complete` methods,
     *                                       or a callback function.
     * @returns {Function} A function to unsubscribe the observer.
     */
    subscribe(observer) {
        return this._subscribeCore(observer);
    }

    /**
     * Provides a subscription interface that skips the immediate emission of the current value.
     * Implemented via the RxJS `skip(1)` operator.
     *
     * @returns {Observable} An observable that skips the initial value.
     */
    get skip() {
        return this.pipe(skip(1));
    }

    /**
     * Subscribes an observer to receive only one emission, then automatically unsubscribes.
     *
     * @param {Object|Function} observer - An observer or callback function.
     * @returns {Subscription} The RxJS subscription that will complete after one emission.
     */
    subscribeOnce(observer) {
        return this.toObservable().pipe(take(1)).subscribe(observer);
    }

    /**
     * Provides a subscription interface for a one-time subscription.
     * This returns an observable that emits a single value and then completes.
     *
     * @returns {Observable} An observable that completes after one emission.
     */
    get once() {
        return this.toObservable().pipe(take(1));
    }


    /**
     * Notifies subscribers of an error.
     *
     * @param {Error} err - The error to notify.
     */
    error(err) {
        this._notifyAll('error', err);
    }

    /**
     * Completes the node by notifying subscribers, clearing subscribers, and unsubscribing from dependencies.
     *
     * @returns {void}
     */
    complete() {
        this._completed = true;
        this._notifyAll('complete');
        this.subscribers.clear();
        this._unsubscribeDependencies();
        // Cancel any inner observable subscription if present.
        if (this._observableSubscription) {
            this._observableSubscription.unsubscribe();
            this._observableSubscription = null;
        }
    }

    /**
     * Internal method to update the node's value and notify subscribers if the value has changed.
     * Uses deepEqual to compare objects and arrays for nested changes.
     *
     * @param {*} newValue - The new value to set.
     * @private
     */
    _setValue(newValue) {
        const prevValue = this.value;
        this.value = newValue;
        // console.log("Computed node update:", { prevValue, newValue });
        if (!deepEqual(prevValue, newValue)) {
            this._notifyAll('next', newValue);
        }
    }

    /**
     * Core subscription method that adds an observer and handles immediate emission of the current value.
     *
     * @param {Object|Function} observer - An observer or a callback function.
     * @returns {Function} A function to unsubscribe the observer.
     * @private
     */
    _subscribeCore(observer) {
        if (this._completed) {
            if (typeof observer.complete === 'function') observer.complete();
            return () => {};
        }

        const subscriber = this._initializeObserver(observer);
        this.subscribers.add(subscriber);

        // Immediately emit the current value if no error has occurred.
        if (typeof subscriber.next === 'function') {
            if (this._error !== undefined) {
                // If an error occurred, notify the error immediately.
                if (typeof subscriber.error === 'function') {
                    subscriber.error(this._error);
                }
                subscriber.closed = true;
            } else {
                subscriber.next(this.value);
                subscriber.lastEmitted = this.value;
            }
        }

        if (this.fn && (!this._dependencySubscriptions || !this._dependencySubscriptions.length)) {
            this._dependencySubscriptions = this._initializeDependencySubscriptions(this.dependencies);
            ReactiveNode.scheduleUpdate(this);
        }

        // Return an unsubscribe function.
        const unsubscribe = () => {
            subscriber.closed = true;
            this.subscribers.delete(subscriber);
            if (this.subscribers.size === 0) this._unsubscribeDependencies();
        };
        unsubscribe.unsubscribe = unsubscribe;
        return unsubscribe;
    }

    /**
     * Normalizes an observer input into an observer object with `next`, `error`, and `complete` methods.
     * If the observer already looks like a proper RxJS subscriber (i.e. it has a _next method),
     * it is returned as is.
     *
     * @param {Object|Function} observer - The observer or callback function.
     * @returns {Object} The normalized observer.
     * @private
     */
    _initializeObserver(observer) {
        // If the observer looks like a proper RxJS Subscriber, return it directly.
        if (observer && typeof observer._next === 'function') {
            return observer;
        }
        // If no observer is provided, default to an empty object.
        if (!observer) {
            observer = {};
        }
        // If observer is a function, wrap it.
        if (typeof observer === 'function') {
            observer = { next: observer };
        }
        // Provide default no-op functions for any missing handlers.
        return {
            next: observer.next || (() => {}),
            error: observer.error || (() => {}),
            complete: observer.complete || (() => {}),
            closed: false,
            lastEmitted: undefined,
            errorNotified: false
        };
    }


    /**
     * Notifies all subscribers of a specific event type.
     *
     * @param {string} type - The type of event ('next', 'error', or 'complete').
     * @param {*} [value] - The value or error to pass to the subscribers.
     * @private
     */
    _notifyAll(type, value) {
        this.subscribers.forEach((subscriber) => {
            if (subscriber.closed) return;

            // Ensure all updates get processed in order, even in batch mode
            queueMicrotask(() => {
                if (type === 'error' && !subscriber.errorNotified && typeof subscriber.error === 'function') {
                    subscriber.error(value);
                    subscriber.errorNotified = true;
                    subscriber.closed = true;
                } else if (type === 'next') {
                    // Store all emitted values to prevent loss
                    subscriber.lastEmittedQueue = subscriber.lastEmittedQueue || [];
                    subscriber.lastEmittedQueue.push(value);

                    while (subscriber.lastEmittedQueue.length > 0) {
                        const nextValue = subscriber.lastEmittedQueue.shift();
                        subscriber.next?.(nextValue);
                        subscriber.lastEmitted = nextValue;
                    }
                } else if (type === 'complete' && typeof subscriber.complete === 'function') {
                    subscriber.complete();
                    subscriber.closed = true;
                }
            });
        });
    }

    /**
     * Subscribes to all dependency nodes so that any change triggers an update of this node.
     *
     * @param {Array<ReactiveNode>} dependencies - The dependencies to subscribe to.
     * @returns {Array<Function>} An array of unsubscribe functions for each dependency.
     * @private
     */
    _initializeDependencySubscriptions(dependencies) {
        return dependencies.map((dep) =>
            dep.subscribe(() => ReactiveNode.scheduleUpdate(this))
        );
    }

    /**
     * Unsubscribes from all dependency subscriptions.
     *
     * @private
     */
    _unsubscribeDependencies() {
        this._dependencySubscriptions?.forEach((unsubscribe) => unsubscribe());
        this._dependencySubscriptions = [];
    }

    /**
     * Adds one or more dependencies to this computed node.
     * Subscribes to each dependency so that changes will trigger an update.
     *
     * @param {ReactiveNode|ReactiveNode[]} dep - A dependency node or an array of dependency nodes to add.
     * @throws {Error} if this node is not computed.
     */
    addDependency(dep) {
        if (!this.fn) {
            throw new Error("Cannot add dependency to a non-computed node.");
        }
        if (Array.isArray(dep)) {
            dep.flat().forEach(d => this.addDependency(d));
            return;
        }
        // Prevent duplicates.
        if (this.dependencies.includes(dep)) return;

        this.dependencies.push(dep);
        // Subscribe to the new dependency.
        const unsubscribe = dep.subscribe(() => ReactiveNode.scheduleUpdate(this));
        // Ensure _dependencySubscriptions is defined.
        if (!this._dependencySubscriptions) {
            this._dependencySubscriptions = [];
        }
        this._dependencySubscriptions.push(unsubscribe);
        // Recompute the node's value now that a new dependency is present.
        this.compute();
    }

    /**
     * Removes one or more dependencies from this computed node.
     * Unsubscribes from each dependency and updates the computed value.
     *
     * @param {ReactiveNode|ReactiveNode[]} dep - A dependency node or an array of dependency nodes to remove.
     * @throws {Error} if this node is not computed.
     */
    removeDependency(dep) {
        if (!this.fn) {
            throw new Error("Cannot remove dependency from a non-computed node.");
        }
        if (Array.isArray(dep)) {
            dep.flat().forEach(d => this.removeDependency(d));
            return;
        }
        const index = this.dependencies.indexOf(dep);
        if (index === -1) return;

        // Remove the dependency.
        this.dependencies.splice(index, 1);
        // Remove and call the corresponding unsubscribe function.
        if (this._dependencySubscriptions && index < this._dependencySubscriptions.length) {
            const unsubscribe = this._dependencySubscriptions[index];
            unsubscribe();
            this._dependencySubscriptions.splice(index, 1);
        }
        // Recompute the node's value now that the dependency is gone.
        this.compute();
    }

    /**
     * Returns an array of the current dependency node IDs.
     * @returns {Array<string>} An array of dependency node IDs.
     */
    listDependencies() {
        return this.dependencies.map(dep => dep.id);
    }

    /**
     * Helper to subscribe to an observable value.
     * If there is an existing observable subscription, it is canceled.
     * The observable’s emissions update the node’s value.
     *
     * @param {Observable} obs - The observable to subscribe to.
     * @private
     */
    _subscribeToObservable(obs) {
        if (this._observableSubscription) {
            this._observableSubscription.unsubscribe();
        }
        this._observableSubscription = obs.subscribe(
            value => this._setValue(value),
            err => this.error(err),
            () => this.complete()
        );
    }

    /**
     * Converts this ReactiveNode into an RxJS Observable.
     *
     * This observable emits the node's current value and subsequent updates.
     *
     * @returns {Observable<*>} An RxJS observable representing this node.
     */
    toObservable() {
        return new Observable((subscriber) => {
            // Emit the current value (if defined).
            if (this.value !== undefined) {
                subscriber.next(this.value);
            }
            // Subscribe to the node updates.
            const unsubscribe = this.subscribe({
                next: (value) => subscriber.next(value),
                error: (err) => subscriber.error(err),
                complete: () => subscriber.complete()
            });
            // Return a teardown function to unsubscribe.
            return unsubscribe;
        });
    }

    /**
     * Triggers an update for the node.
     * For computed nodes, this re-computes the value.
     * For stateful (non-computed) nodes, if a function is provided,
     * it calls that function with the current value and updates the node with the result.
     *
     * @param {Function} [fn] - An update function that takes the current value and returns the new value.
     */
    update(fn) {
        if (this.fn) {
            // For computed nodes, simply re-compute.
            this.compute();
        } else {
            // For stateful nodes, expect an update function.
            if (typeof fn === 'function') {
                const newValue = fn(this.value);
                this.set(newValue);
            } else {
                throw new Error("update() requires a function argument for stateful nodes");
            }
        }
    }
}

export { ReactiveNode, setIdGenerator };
