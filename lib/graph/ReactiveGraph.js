 ...
 * const node1 = createNode(4);         // node1.key is a Uint8Array
 * const node2 = createNode(() => 6);     // node2.key is a Uint8Array
...
class ReactiveGraph {
     ...
     */
    constructor(config = {}) {
        this.nodes = new BufferMap(); // Map<byte-array, node>
        this.edges = new BufferMap(); // Map<byte-array, Set<byte-array>>
    }

    /**
     * Returns a node's id as a z32‑encoded string.
     *
     * @param {Uint8Array} key - The node's key.
    ...
    /**
     * Resolves a node reference into a byte-array key.
     *
     * Expects one of:
     *     * - A string: assumed to be a z32-encoded id and decoded into a Uint8Array.
     *      * - A Uint8Array: returned as is.
     *      * - A dagify node object: must have isDagifyNode===true and a `key` property (a Uint8Array).
     * @private
     * @param {string|Uint8Array|Object} ref - A string, Uint8Array, or dagify node object.
     * @returns {Uint8Array} The resolved byte-array key.
     ...
     *
     * @param {string|Uint8Array|Object} nodeRef - The node reference to remove.
