...
class ReactiveGraph {
    ...
    connect(srcRef, tgtRef) {
        // Normalize the source and target references to arrays.
        const srcRefs = Array.isArray(srcRef) ? srcRef : [srcRef];
        const tgtRefs = Array.isArray(tgtRef) ? tgtRef : [tgtRef];

        // Iterate over each combination of source and target.
        for (const src of srcRefs) {
            ...
            for (const tgt of tgtRefs) {
                ...
                // Connect the nodes by adding the target id to the source's edge set.
                this.edges.get(srcId).add(tgtId);

                // If the target node is computed (has a function), add the dependency.
                const tgtNode = this.nodes.get(tgtId);
                const srcNode = this.nodes.get(srcId);
                if (tgtNode.fn) {
