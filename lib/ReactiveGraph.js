 ...
 * const node1 = createNode(4);
 * const node2 = createNode(() => 6);
 * // use the node's id
 * graph.addNode(node1);
 * // define an id rather than using the node's id.
 * graph.addNode("six", node2);
...
class ReactiveGraph {
    ...
    /**
     * Retrieves an existing node by its ID or adds a new node to the graph if it doesn't exist.
     *
     * Supports two input formats:
     * 1. A single dagify node object (which must have `isDagifyNode === true` and an `id` property).
     * 2. A tuple format where the first argument is an identifier (string or Buffer) and the second argument
     *    is a dagify node object (which must have `isDagifyNode === true`).
     *
     * If the node already exists in the graph, this method returns the existing node. Otherwise, it adds
     * the node to the graph (setting up its edges) and returns the new node.
     *
     ...
     */
    upsertNode(nodeOrId, maybeNode) {
        let id, node;
        if (maybeNode === undefined) {
            // Single argument mode: expecting a dagify node object.
            ...
            // Tuple mode: first argument is an identifier, second is the node.
     ...
     * 2. By passing an identifier (string or Buffer) as the first argument and the dagify node as the second argument.
     *
     * In the tuple form, the provided identifier is used to update the node’s internal ID.
     *
     * @param {string|Buffer|Object} nodeOrId - Either a dagify node object or an identifier for the node.
     * @param {Object} [maybeNode] - The dagify node object if an identifier is provided as the first argument.
     * @throws {Error} If the node is invalid or a node with the same key already exists.
    ...
    addNode(nodeOrId, maybeNode) {
        ...
        if (maybeNode === undefined) {
            ...
            // Update the node's internal ID to match the provided custom id.
            // This ensures consistency when later referencing the node.
            node._id = id;
        }
        if (this.nodes.has(id)) {
     ...
     *
     * This method iterates over the provided array and adds each node to the graph
     * by invoking the {@link addNode} method. Each node entry can be provided in one of two formats:
     *
     * 1. **Single node object:** A dagify node object that must have `isDagifyNode === true`
     *    and an `id` property.
     * 2. **Tuple format:** An array with two elements where the first element is an identifier
     *    (a string or Buffer) and the second element is a dagify node object (which must have
     *    `isDagifyNode === true`).
     *
     * @param {(Object|[string|Buffer, Object])[]} nodes - An array of nodes or node entries to add to the graph.
     * @throws {Error} If any node is invalid or if a node with the same key already exists.
     */
    addNodes(nodes) {
        for (const node of nodes) {
            this.addNode(...(Array.isArray(node) ? node : [node]));
        }
    }
    ...
    connect(srcRef, tgtRef) {
        // Normalize source and target references to arrays.
        const srcRefs = Array.isArray(srcRef) ? srcRef : [srcRef];
        const tgtRefs = Array.isArray(tgtRef) ? tgtRef : [tgtRef];

        // Iterate over each source-target combination.
        for (const src of srcRefs) {
            const srcId = this._resolveId(src);
            for (const tgt of tgtRefs) {
                const tgtId = this._resolveId(tgt);
                ...
                // For computed nodes, add the dependency.
                const tgtNode = this.nodes.get(tgtId);
                const srcNode = this.nodes.get(srcId);
                if (tgtNode.fn) {
                    // Check if the dependency structure is positional (array) or named (object).
                    if (Array.isArray(tgtNode.normalizedDeps)) {
                        // Positional: Use addDependency.
                        tgtNode.addDependency(srcNode);
                    } else if (tgtNode.normalizedDeps && typeof tgtNode.normalizedDeps === 'object') {
                        // Named: Use updateDependencies.
                        // Use the source node's ID as the key.
                        const depKey = srcId;
                        // Only add if not already present.
                        if (!(depKey in tgtNode.normalizedDeps)) {
                            tgtNode.updateDependencies(deps => ({ ...deps, [depKey]: srcNode }));
                        }
                    }
                }
            }
    ...
    /**
     * Overloaded disconnect method.
     *
     * Both arguments can be a single node (or node id) or an array of nodes.
     * Each node in the source collection will be disconnected from every node
     ...
     * @returns {Object|null} The first matching node, or null if no node matches.
     */
    findNode(predicate) {
