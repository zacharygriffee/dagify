...
import {keyToZ32} from "../util/keyGenerator.js";
...
class ReactiveNode extends Subject {
    ...
    constructor(fnOrValue, dependencies, config) {
        ...
        // Use the provided keyPair from config if available; otherwise generate one.
        this.#keyPair = config.keyPair || crypto.keyPair();
    ...
    compute() {
        ...
        try {
            ...
            // (Assuming that each reactive dependency node has an _error property if it errored.)
            const reactiveNodes = Array.isArray(this.normalizedDeps)
                ? this.normalizedDeps
                : Object.values(this.normalizedDeps);
            for (const dep of reactiveNodes) {
                if (dep._error !== undefined) {
                    // Propagate the error from the dependency.
                    this.error(dep._error);
                    return;
                }
            }
