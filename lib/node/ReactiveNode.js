...
class ReactiveNode extends Subject {
     ...
     * @param {(ReactiveNode | CompositeNode | Function | Promise | Observable) |
     *         (ReactiveNode | CompositeNode | Function | Promise | Observable)[] |
     *         { [key: string]: ReactiveNode | CompositeNode | Function | Promise | Observable }} [dependencies=[]] - The dependencies for computed nodes.
    ...
    static scheduleUpdate(node) {
        if (node.disableBatching) {
            // Skip batching, recompute immediately
            node.compute();
            return;
        }
    ...
    _notifyAll(type, value) {
        ...
        if (this.disableBatching) {
            // Synchronous call to each subscriber
