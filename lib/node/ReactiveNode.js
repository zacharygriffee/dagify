...
class ReactiveNode extends Subject {
    ...
    constructor(fnOrValue, dependencies, config) {
        ...
        this.enableActivityThresholding = config.useNeuronElevation ?? false;
        if (this.enableActivityThresholding) {
            ...
            // Store the timer handle for cleanup.
    ...
    // --- Neuron-Inspired Methods ---

    /**
     * Call this method when the node is "visited".
     * It increases the activity level and triggers an update if the threshold is reached.
     */
    visit() {
        if (!this.enableActivityThresholding) {
            // Fallback: immediate compute if not using neuron elevation.
            this.compute();
            return;
        }
        this.activityLevel++;
        ...
        if (this.activityLevel >= this.activationThreshold) {
            // Node "fires": compute and reset activity.
            this.compute();
            this.activityLevel = 0;
        }
    }
    ...
    /**
     * Starts a decay timer to reduce the activity level when the node is not being visited.
     */
    startDecay() {
