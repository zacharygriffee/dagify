...
import { normalizeDependency, setNormalizeDependencyCtor } from "./normalizeDependency.js";
...
import b4a from "b4a";
import { shouldFailFast } from "./failFast.js";
import { getSchedulers } from "../util/schedulers.js";
import { setFromObservableNodeCtor } from "../util/fromObservable.js";
...
class ReactiveNode extends Subject {
     ...
     * @param {string} [config.valueEncoding="any"] - The encoding for the value when calling 'encodeForSink' to get a Uint8Array for serialization. If the node gets a Uint8Array as a
    ...
    constructor(fnOrValue, dependencies, config) {
        ...
        this._failFast = config.failFast;
        this._failFastPredicate = typeof config.failFastPredicate === "function" ? config.failFastPredicate : null;
        this._compare = typeof config.compare === "function" ? config.compare : null;
        this._updateScheduler = config.updateScheduler;
        this._notifyScheduler = config.notifyScheduler;
        ...
        if (isComputed) {
            ...
            if (isRxObservable(fnOrValue)) {
                ...
                this._isAsync = false;
                // For static nodes, assign through the internal setter to keep snapshots in sync.
                this._setValue(fnOrValue === undefined ? NO_EMIT : fnOrValue, true);
            }
        }
     ...
     * Gets the node's key.
     *
     * @returns {Uint8Array} The node's key.
     */
    get key() {
        return this._key;
    }
     ...
     * Sets the node's key.
     *
     * @param {Uint8Array} value - A 32-byte Uint8Array representing the new key.
     * @throws {Error} Throws an error if the key is not a 32-byte Uint8Array.
     */
    set key(value) {
        if (value.byteLength !== 32) throw new Error("Key must be 32 byte Uint8Array.");
        this._key = value;
    }
    ...
    /**
     * Provides an observable view of the node for FRP-style composition.
     *
     * @returns {Observable} Observable that emits the node's values.
     */
    get stream() {
    ...
    get value() {
        if (this._encodedValue !== null &&
            ...
            if (b4a.isBuffer(this._encodedValue)) {
                try {
                ...
                // If the encoded value isn't a Uint8Array, we defer to type checking.
                return this._value;
            }
        }
        return this._value;
     ...
     */
    set value(newValue) {
        // If newValue is a Uint8Array and a valid encoding is provided, decode it first.
        if (b4a.isBuffer(newValue) &&
            this._valueEncoding &&
            this._valueEncoding !== "any") {
            try {
        ...
        if (!this._validateType(newValue)) {
            ...
            this.error(typeErr);
            // Do not update the value: it remains as NO_EMIT.
            return;
        }
    ...
    static scheduleUpdate(node) {
        const shouldAggregate = node.enableActivityThresholding === true;
        if (node.disableBatching) {
            // debug
            // console.log('scheduleUpdate immediate', node.fn?.name);
            shouldAggregate ? node.visit() : node.compute();
            return;
        }
        ...
        if (!ReactiveNode.updating) {
            ...
            const scheduler = node._updateScheduler || getSchedulers().update;
            scheduler.schedule(() => {
                ReactiveNode.pendingUpdates.forEach(n => {
                    n.enableActivityThresholding ? n.visit() : n.compute();
                });
    ...
    static batch(fn) {
        ...
        try {
            ...
            const updates = Array.from(ReactiveNode.pendingUpdates);
            ReactiveNode.pendingUpdates.clear();
            try {
                updates.forEach(node => {
                    if (node.enableActivityThresholding) {
                        node.visit();
                    } else {
                        node.compute();
                    }
    ...
    compute() {
        ...
        try {
            ...
            console.warn(err);
            this._value = err;
            this._snapshot = cloneForComparison(err);
            this._dependencyErrorSubject.next(err);
            this._error = err;
            this._notifyAll('error', err);
            if (this._shouldFailFast(err)) {
                throw err;
            }
        }
    }
    ...
    _setValue(newValue, forceEmit = false) {
        ...
        if (forceEmit || !this._valuesEqual(prevSnapshot, nextSnapshot)) {
            this._notifyAll('next', newValue);
        }
    }

    /**
     * Compares snapshots using either a user-supplied comparator or the default deep equality.
     * @private
     */
    _valuesEqual(prevSnapshot, nextSnapshot) {
        const comparator = this._compare || deepEqual;
    ...
    _notifyAll(type, value) {
        if (this.disableBatching) {
            ...
            const scheduler = this._notifyScheduler || getSchedulers().notify;
            scheduler.schedule(() => this._flushNotifications());
        }
    }

    /**
     * Flushes pending notifications in a single microtask to reduce per-subscriber scheduling overhead.
     * @private
     */
    _flushNotifications() {
        ...
        for (const entry of queue) {
            const { type, value, subscribers } = entry;
    ...
    _shouldFailFast(err) {
        return shouldFailFast(err, this._failFastPredicate, this._failFast);
    }
...
// Register constructors for helper modules to avoid ESM TDZ cycles.
setNormalizeDependencyCtor(ReactiveNode);
