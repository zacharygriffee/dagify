...
| --- | --- | --- | --- |
| fan-out 1 -> 300 (batched) | ~467 | ~2.7 ms | batched microtask delivery |
| fan-out 1 -> 300 (unbatched) | ~1260 | ~0.87 ms | synchronous notify wins for small graphs |
| fan-out 1 -> 1000 | tbd | tbd | added bench to size large fan-out |
| fan-in 300 -> 1 | ~535 | ~2.2 ms | heavy reduce over 300 inputs |
| fan-in 1000 -> 1 | tbd | tbd | added bench to size large fan-in |
| notify 1000 subs (batched) | ~732 | ~1.4 ms | per-subscriber microtasks |
| notify 1000 subs (unbatched) | ~5700 | ~0.20 ms | sync fan-out, no microtasks |
| queued 100 enqueues, 0ms work | ~49 | ~20.4 ms | serialization overhead only |
| queued 100 enqueues, 1ms work | ~5 | ~200.7 ms | dominated by async payloads |
